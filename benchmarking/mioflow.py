...
# from baseline.MIOFlow_model_revised.running import makeSimulation, trainModel
...
# from plotting.PlottingUtils import computeLatentEmbedding, umapWithPCA
# from plotting.visualization import (
#     computeDrift,
#     plotPredAllTime,
#     plotPredTestTime,
#     plotStream,
#     plotStreamByCellType,
# )
...
# Specify the type of prediction tasks:
# three_interpolation, two_forecasting,
# three_forecasting, remove_recovery
...
# data = ann_data.X
# true_data = [
#     data[np.where(cell_tps == t)[0], :] for t in range(1, n_tps + 1)
# ]  # (# tps, # cells, # genes)
...
#     data=np.concatenate(
#         [np.repeat(train_tps[t], x.shape[0]) for t, x in enumerate(train_data)]
#     )[:, np.newaxis],
#     columns=['samples'],
# )
...
# gae_embedded_dim, encoder_layers, layers, lambda_density = tunedMIOFlowPars(
...
#     n_epochs_emb=1000,
#     samples_size_emb=(50,),
#     gae_embedded_dim=gae_embedded_dim,
...
# generated = makeSimulation(
#     train_df, model, tps, opts, n_sim_cells=2000, n_trajectories=100, n_bins=100
# )
# forward_recon_traj = [
#     pca_model.inverse_transform(generated[i, :, :]) for i in range(generated.shape[0])
# ]
# # ======================================================
...
#     [
#         np.repeat(t, forward_recon_traj[t].shape[0])
#         for t in range(len(forward_recon_traj))
#     ]
# )
...
# state_filename = "{}/{}-{}-MIOFlow-state_dict.pt".
# format(save_dir, data_name, split_type)
