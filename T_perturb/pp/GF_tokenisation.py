...
style.use('default')
style.use("/lustre/scratch123/hgi/projects/healthy_imm_expr/t_generative/T_perturb/T_perturb/pp/mpl_style.mplstyle")
...
#load adata 
adata=sc.read_h5ad("/lustre/scratch123/hgi/projects/healthy_imm_expr/t_generative/data/h5d_files/cytoimmgen.h5ad")
#map ensembl ids to gene names
adata = map_ensembl_to_genename(
    adata, 
    "/lustre/scratch123/hgi/projects/healthy_imm_expr/t_generative/data/h5d_files/phase2_data_qced_cells_cellCycleScored_geneMetadata.csv.gz"
    )
adata.var['ensembl_id'] = adata.var.index

#filter for only DEGs
degs=pd.read_csv("/lustre/scratch123/hgi/projects/healthy_imm_expr/t_generative/generative_modelling_omic_notebooks/pp/res/deg/significant_deg_1.5logfc_0.05padj_hvg_5k.csv")
unique_degs = degs["names"].unique()
#filter adata for only degs
adata = adata[:, adata.var["gene_name"].isin(unique_degs)]

var_to_keep = ['Cell_population', 'Cell_type', 'Time_point','Age', 'Sex','batch','Cell_culture_batch','Phase','Donor']
var_to_keep = {v : v for v in var_to_keep}
...
output_dir = "./res/dataset"
with open(os.path.join(output_dir, "cytoimmgen_tokenised_per_timepoint_length.pkl"), "wb") as f:
    pickle.dump(length, f)

#load pkl file 
with open("/lustre/scratch123/hgi/projects/healthy_imm_expr/t_generative/generative_modelling_omic/Geneformer/geneformer/token_dictionary.pkl", "rb") as f:
...
#load adata
adata=sc.read_h5ad("./res/h5ad_data/cytoimmgen_genotying_subset.h5ad")
#use adata var to map ensembl ids to gene names
ensembl_id_to_genename = dict(zip(adata.var_names, adata.var['gene_name']))
gene_name_list = [[ensembl_id_to_genename.get(i, None) for i in ensembl_ids] for ensembl_ids in ensembl_ids_list]
...
degs_idx_df["Time_point"] = pd.Categorical(degs_idx_df["Time_point"], ["0h", "16h", "40h", "5d"])
#ignore nan values
degs_idx_df[~degs_idx_df['CD69'].isna()]['CD69'].plot(kind='hist', bins=100)
plt.xlabel("rank of degs")
plt.ylabel("Counts")
plt.savefig("./res/tokenised_deg/deg_idx_histogram_CD69.pdf", dpi=300, bbox_inches='tight')
plt.close()
...
plt.ylabel("Counts")
plt.title("IL2RA")
plt.savefig("./res/tokenised_deg/deg_idx_histogram_IL2RA.pdf", dpi=300, bbox_inches='tight')
plt.close()
...
plt.ylabel("Ranks")
plt.title("IL2RA")
plt.savefig("./res/tokenised_deg/deg_idx_violin_IL2RA.pdf", dpi=300, bbox_inches='tight')
plt.close()
