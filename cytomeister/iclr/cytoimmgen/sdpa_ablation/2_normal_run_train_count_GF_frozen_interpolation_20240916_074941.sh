...
#BSUB -o logs/normal_count_GF_frozen_%J.out # output file
#BSUB -e logs/normal_count_GF_frozen_%J.err # error file
#BSUB -M 50000  # RAM memory part 2. Default: 100MB
#BSUB -R 'select[mem>50000] rusage[mem=50000]' # RAM memory part 1. Default: 100MB
#BSUB -J normal_sdpa_cytoimmgen_count_GF_frozen # job name
...
xist, create it with the name $RES_NAME
mkdir -p $RES_DIR/$RES_NAME
# Get the current timestamp
TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
# copy the current script to the result directory
cp $0 $RES_DIR/$RES_NAME/normal_run_train_count_GF_frozen_interpolation_$TIMESTAMP.sh
echo "Copying script to $RES_DIR/$RES_NAME/normal_run_train_count_GF_frozen_interpolation_$TIMESTAMP.sh"
...
--ckpt_masking_path "./T_perturb/cytomeister/iclr/sdpa_ablation/checkpoints/20240916_0058_cellgen_train_masking_lr_0.0001_wd_0.0001_batch_256_mlmp_0.15_tp_1-3_s_42-epoch=09.ckpt" \
